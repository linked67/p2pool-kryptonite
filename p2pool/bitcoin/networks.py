...
kryptonite=math.Object(
        ...
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'kryptoniteaddress' in (yield bitcoind.rpc_help()) and
        ...
        BLOCK_PERIOD=31, # s
        SYMBOL='KRYP',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'kryptonite') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/kryptonite/') if platform.system() == 'Darwin' else os.path.expanduser('~/.kryptonite'), 'kryptonite.conf'),
